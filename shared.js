...
function bindVnPhoneInput(el) {
  ...
  el.addEventListener('blur', () => {
    if (!el.value) {
      el.setCustomValidity(el.required ? 'Vui lòng nhập số điện thoại' : '');
      return;
    }
    el.setCustomValidity(
      isValidVietnamPhone(el.value)
        ? ''
        : 'Số điện thoại VN: 10 số, đầu 03/05/07/08/09 (vd: 0901234567)'
    );
  });
}
...
function bindEmailInput(el) {
  ...
  el.addEventListener('blur', () => {
    if (!el.value) {
      el.setCustomValidity(el.required ? 'Vui lòng nhập email' : '');
      return;
    }
    el.setCustomValidity(
      isValidEmail(el.value)
        ? ''
        : 'Email không đúng định dạng (vd: ten@gmail.com)'
    );
  });
  el.addEventListener('input', () => {
...
function renderCard(p, siteUrl) {
  ...
  return `
<div class="product-card" onclick="window.location.href='product-demo.php?id=${p.id}'" style="cursor: pointer;">
...
function buyNow(id, name, price, img) {
    // Lưu riêng - KHÔNG đụng giỏ hàng (fsw-cart)
    localStorage.setItem('fsw-buynow', JSON.stringify({ id, name, price, img, qty: 1 }));
    window.location.href = 'checkout-buynow.php';
}
