...
<script>
  ...
  function renderGrid(targetId, list) {
    ...
    grid.innerHTML = list.map(p => `
      <div class="hero-card" onclick="window.location.href='product-demo.php?id=${p.id}'" style="cursor: pointer;">
        ...
        <div class="hc-name">${p.name} ${p.isNew?'<span style="background:red;color:white;font-size:10px;padding:2px 5px;border-radius:3px;margin-left:5px">MỚI</span>':''}</div>
        <div class="hc-price">${fmtVND(p.price)} ${p.oldPrice && p.oldPrice > p.price ? `<span style="text-decoration:line-through;color:#aaa;font-size:12px;margin-left:8px">${fmtVND(p.oldPrice)}</span>`:''}</div>
