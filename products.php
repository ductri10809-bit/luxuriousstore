...
<script>
  ...
  function searchProducts() {
    ...
    if (list.length === 0) {
        document.getElementById('productGrid').innerHTML =
            '<p style="color:var(--text-muted);font-size:14px;grid-column:1/-1;padding:32px 0">Không tìm thấy sản phẩm phù hợp với "<b>' + q + '</b>"</p>';
    } else {
        renderGrid('productGrid', list);
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Lấy ô search từ nav (class="search-box") và gán id để JS dùng được
    const navSearch = document.querySelector('.search-box');
    if (navSearch) {
        ...
        navSearch.addEventListener('input', () => searchProducts());
        // Bỏ redirect Enter, tìm trực tiếp trên trang
