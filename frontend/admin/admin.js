  ...
  try {
    ...
    table.innerHTML = products.map(product => `
      <tr data-id="${product.id}">
        ...
        <td>${product.ten_danh_muc || product.category_id || ''}</td>
        <td><button type="button" class="btn btn-secondary btn-delete-product" data-id="${product.id}">Xóa</button></td>
      </tr>
    `).join('');
  } catch (err) {
    table.innerHTML = `<tr><td colspan="6">Lỗi: ${err.message}</td></tr>`;
  }
}
  ...
  try {
    ...
    table.innerHTML = orders.map(order => `
      <tr data-id="${order.order_id}">
        ...
        <td><button type="button" class="btn btn-secondary btn-update-order">Cập nhật</button></td>
      </tr>
    `).join('');
  } catch (err) {
    table.innerHTML = `<tr><td colspan="5">Lỗi: ${err.message}</td></tr>`;
